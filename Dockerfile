...
RUN DOWNLOAD_URL="https://download.java.net/java/GA/jdk${JAVA_VERSION}/9/GPL/openjdk-${JAVA_VERSION}.0.2_linux-x64_bin.tar.gz" \
    ...
    && rm -rf "${TMP_DIR}" \
    && chown -R airflow:airflow "${JAVA_HOME}" \
    && java --version

USER airflow
RUN pip install apache-airflow-providers-celery apache-airflow-providers-apache-spark pyspark==3.1.3 apache-airflow-providers-openlineage redis boto3